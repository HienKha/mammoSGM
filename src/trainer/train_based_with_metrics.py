...
import torch
import pandas as pd
import albumentations as A  # For normalization dictionary
import csv
from src.data.based_data import get_dataloaders
from src.data.dataloader import load_metadata

from src.data.dataloader import load_metadata
from src.trainer.engines import train_model, evaluate_model, evaluate_model_and_save_metrics
...
def run_test(
    ...
    pretrained_model_path=None,
    target_column=None,  # thêm target_column
    setting=None,  # thêm setting
    backbone_name=None,  # thêm backbone_name
):
    ...
    saved_csv_file = os.path.join(output, "test_metrics.csv")
...
if __name__ == "__main__":
    ...
    setting = get_arg_or_config(args.setting, config.get("setting"), None)
    backbone_name = get_arg_or_config(args.backbone_name, config.get("backbone_name"), None)
